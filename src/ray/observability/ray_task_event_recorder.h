...
class RayTaskEventRecorder : public RayEventRecorderBase {
 ...
 private:
  ...
  // (claude) Move the buffered events, and the dropped task attempts to report, out of
  // the buffers so that the caller can group, serialize and send them without holding
  // mutex_. Events belonging to a dropped attempt are discarded so that a task attempt is
  // either sent in full or not at all.
